...
"$VLLVM_CLANG" "${EXTRA_ARGS[@]}" -O0 -S -emit-llvm -lvars "$SRC" \
...
"$VLLVM_CLANG" "${EXTRA_ARGS[@]}" -O0 "$SRC" -o "$OUT_DIR/test_lvars_base"
"$VLLVM_CLANG" "${EXTRA_ARGS[@]}" -O0 -lvars "$SRC" -o "$OUT_DIR/test_lvars"
