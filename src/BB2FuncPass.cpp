...
namespace {
// 提取规模上限：控制单个函数的变更幅度，防止把函数抽干或过度膨胀。
...
// 安全闸 1：单个块存在以下状态时不能参与提取，否则 CodeExtractor 会产生
// 非法 IR 或直接崩溃。
...
// 动态栈状态（VLA/stacksave/stackrestore）跨越提取边界后恢复语义会被破坏。
bool hasDynamicStackState(BasicBlock &BB) {
...
// 安全闸 2：区域必须是单入口，区域外前驱只允许打在 header 上。
...
// 线性胶水边：无条件 br + 单前驱 + 后继无 PHI（避免引入 demote）。
...
// 语义指令是候选评分与门槛的度量单位：排除 terminator/PHI/调试信息/
// alloca/cast/GEP，只统计真实计算，保证提取出的 helper 有实际内容。
...
// 评分 = 语义密度 + 多块奖励 + 形态奖励；重塑过的形态优先于原始形态。
...
// 塑形第二步：把粘合后的大块在随机语义点重新切开，使块边界偏离源码结构。
bool splitGluedBlock(BasicBlock *BB, std::default_random_engine &RNG,
  ...
  unsigned DesiredSplits = std::min<unsigned>(MaxSplits, 1 + RNG() % MaxSplits);
  if (DesiredSplits == 0)
    return false;
...
bool mergeWindow(ArrayRef<BasicBlock *> Window) {
  ...
  for (unsigned I = 1, E = Window.size(); I != E; ++I)
    if (!isLinearEdge(Window[I - 1], Window[I]))
      return false;
  for (unsigned I = 1, E = Window.size(); I != E; ++I)
    if (!MergeBlockIntoPredecessor(Window[I]))
      return false;
  return true;
}

// 塑形第一步：随机取一段线性窗口整体粘成一个大块，交给 splitGluedBlock
// 重新切分，制造与源码结构无关的区域边界。
bool shapeOneLinearWindow(LinearChain &Chain, std::default_random_engine &RNG,
                          ShapedChain &Out) {
  if (Chain.Blocks.size() < MinGlueBlocks)
    return false;
  ...
  if (MaxLen > MinGlueBlocks)
    Len += RNG() % (MaxLen - MinGlueBlocks + 1);

  unsigned StartLimit = Chain.Blocks.size() - Len + 1;
  unsigned Start = StartLimit > 1 ? RNG() % StartLimit : 0;

  SmallVector<BasicBlock *, MaxGlueBlocks> Window;
  for (unsigned I = 0; I < Len; ++I)
    Window.push_back(Chain.Blocks[Start + I]);

  if (semanticInstCount(Window) < MinSplitSemanticInsts)
    return false;
  if (!mergeWindow(Window))
    return false;

  // 粘合后窗口收缩成 Window[0] 一个块。
  return splitGluedBlock(Window.front(), RNG, Out.Blocks);
}

bool shapeLinearWindows(Function &F, std::default_random_engine &RNG,
  ...
  for (LinearChain &Chain : Chains) {
    if (Changed >= MaxLinearWindows)
      break;
    ShapedChain SC;
    if (shapeOneLinearWindow(Chain, RNG, SC)) {
      ++Changed;
...
bool shapeDenseBlocks(Function &F, std::default_random_engine &RNG,
                      SmallVectorImpl<ShapedChain> &Shaped,
                      DenseSet<BasicBlock *> &ShapedBlocks) {
...
void collectShapedCandidates(ArrayRef<ShapedChain> Shaped,
  ...
  for (const ShapedChain &SC : Shaped) {
    ...
    if (SC.Blocks.size() < 3)
      continue;
    // 子链候选刻意排除"从 0 到底"的整链区间：把整条自然链原样提走
    // 违背重塑目的，只取中间片段。
    for (unsigned Start = 0, E = SC.Blocks.size(); Start != E; ++Start) {
      for (unsigned Len = 2; Len <= MaxRegionBlocks && Start + Len <= E;
           ++Len) {
...
void extendLinearArm(BasicBlock *Start, BasicBlock *Header, DominatorTree &DT,
                     ...
                     SmallVectorImpl<BasicBlock *> &Blocks) {
  unsigned Limit = 1 + RNG() % MaxArmBlocks;
...
// 分支臂候选：header + 随机先试一条臂，臂不可用再试另一条，提取形状
// 带随机性以抵抗签名匹配。
std::optional<RegionCandidate>
buildBranchArmRegion(BasicBlock &Header, DominatorTree &DT,
                     std::default_random_engine &RNG) {
  auto *BI = dyn_cast<BranchInst>(Header.getTerminator());
  if (!BI || !BI->isConditional())
    return std::nullopt;
  if (unsafeReason(Header))
    return std::nullopt;
...
// switch 子集候选：随机挑部分后继块进区域；MaxPick 被 coversTooMuch
// 的上限封顶，避免唯一候选被拒。
std::optional<RegionCandidate>
buildSwitchSubsetRegion(BasicBlock &Header, DominatorTree &DT,
                        std::default_random_engine &RNG) {
  ...
  if (MaxPick < MinPick)
    return std::nullopt;
  unsigned Pick = MinPick + (MaxPick > MinPick ? RNG() % (MaxPick - MinPick + 1)
                                               : 0);
...
// CodeExtractor 会把源函数的 fn attributes 复制给 helper；必须摘除全部
// vllvm 标记，否则函数适配器遍历到运行期间新增的 helper 时会把它再次
// 当成混淆目标，造成递归提取。
void stripInheritedVLLVMAttrs(Function &F) {
  static const StringRef Kinds[] = {
      "vllvm.obfuscate", "vllvm.enstr", "vllvm.vmfla", "vllvm.fla",
      "vllvm.icall",     "vllvm.ibr",   "vllvm.lvars", "vllvm.bcf",
      "vllvm.vmp",       "vllvm.bb2func"};
  for (StringRef Kind : Kinds)
    F.removeFnAttr(Kind);
}

// 提取一个候选区域；成功返回 helper 函数（已统一命名并打属性），失败返回
// nullptr。
Function *extractRegion(Function &F, DominatorTree &DT,
                        ArrayRef<BasicBlock *> Blocks, CandidateKind Kind,
                        unsigned HelperIndex, bool ChainToMerge) {
  if (Blocks.empty())
    return nullptr;
  for (BasicBlock *BB : Blocks)
    if (BB->getParent() != &F)
  ...
  stripInheritedVLLVMAttrs(*Extracted);
  // 标记 helper 身份，防止后续 pass 或重复调度把它再当普通函数处理。
  Extracted->addFnAttr("vllvm.bb2f");
  // 调用边界必须留在 IR 中：后续 merge 依赖这些真实 call 才能融合，
  // 内联回去等于白做；OptimizeNone 防止后续优化把切分的块重新合并。
  Extracted->removeFnAttr(Attribute::AlwaysInline);
  Extracted->addFnAttr(Attribute::NoInline);
  Extracted->addFnAttr(Attribute::OptimizeNone);
  Extracted->setName(
      (Twine("vllvm.bb2f.") + F.getName() + "." + Twine(HelperIndex)).str());
  if (ChainToMerge) {
    // 链路模式：给 helper 打 merge 标记，值为源函数名，模块级 MergePass
    // 按该值分组融合。
    Extracted->addFnAttr("vllvm.merge", F.getName());
  }
  return Extracted;
}

void collectRegionCandidates(Function &F, DominatorTree &DT,
                             std::default_random_engine &RNG,
                             SmallVectorImpl<RegionCandidate> &Candidates) {
  for (BasicBlock &BB : F) {
...
void collectSingleBlockCandidates(Function &F, DominatorTree &DT,
                                  SmallVectorImpl<RegionCandidate> &Candidates) {
  for (BasicBlock &BB : F) {
...
bool runBB2FuncOnFunction(Function &F, bool ChainToMerge) {
  if (F.isDeclaration() || F.empty())
    return false;

  // 有 personality 的 EH 函数整体跳过：提取会破坏 funclet 结构。
  if (F.hasPersonalityFn())
    return false;

  // 防御：bb2func 自己提取的 helper 不再二次处理。
  if (F.hasFnAttribute("vllvm.bb2f"))
    return false;

  bool Modified = false;
  CryptoUtils Crypto(F.getParent());
  std::default_random_engine RNG(Crypto.getRandom32());
  ...
  collectSingleBlockCandidates(F, DT, Candidates);
  ...
  for (RegionCandidate &Candidate : Candidates) {
    ...
    if (Function *Helper = extractRegion(F, DT, Candidate.Blocks,
                                         Candidate.Kind, Extracted,
                                         ChainToMerge)) {
      (void)Helper;
      // 提取会改动 CFG，重建 DT 保证后续候选拿到最新支配关系。
      DT.recalculate(F);
  ...
  if (Extracted != 0)
    errs() << "[vllvm] BB2FuncPass:" << F.getName()
           << ":extracted=" << Extracted << "\n";
  return Modified;
}
} // namespace
