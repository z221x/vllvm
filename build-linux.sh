...
LLVM_BUILD=${LLVM_BUILD:-"$ROOT_DIR/build/llvm-linux"}
LLVM_INSTALL_PREFIX=${LLVM_INSTALL_PREFIX:-"$ROOT_DIR/out/llvm-linux"}
CC=${CC:-clang}
CXX=${CXX:-clang++}
...
apply_vllvm_patch() {
  if git -C "$LLVM_SRC" apply --reverse --check "$PATCH_FILE" >/dev/null 2>&1; then
    echo "VLLVM clang patch is already applied"
    return
  fi
...
echo "example:"
echo "  $LLVM_BUILD/bin/clang -enstr input.c -o input"
